...
\begin{frame}{Project Motivation \& Context}
\begin{block}{Primary Learning Objective}
Master end-to-end ML/DL workflow for real-world problems in preparation for PhD research
\end{block}
...
\begin{itemize}
    ...
    \item Hands-on learning before PhD enrollment (2025)
    \item Intentionally chose \textbf{challenging dataset} to explore model behaviors
    \item Focus: Understanding, not just performance
\end{itemize}

\column{0.5\textwidth}
\textbf{What Makes It Challenging:}
\begin{itemize}
    \item Kaggle top score: 69.6\%
    \item High irreducible error in data
    \item Weak feature correlations (max 0.118)
    \item Perfect for learning \textbf{what works \& why}
\end{itemize}
\end{columns}
\end{frame}
...
\begin{enumerate}
    \item \textbf{ML Pipeline Mastery}
    \begin{itemize}
        \item Data preprocessing, feature engineering, model selection
        \item Hyperparameter tuning and impact analysis
    \end{itemize}
    
    \item \textbf{Deep Learning Implementation}
    \begin{itemize}
        \item PyTorch from scratch, GPU optimization
        \item 6 different architectures: MLP, ResNet, Attention, Transformers
    \end{itemize}
    
    \item \textbf{Comparative Analysis}
    \begin{itemize}
        \item When to use ML vs DL? Why tree ensembles beat neural nets on tabular data?
    \end{itemize}
    
    \item \textbf{Full-Stack Deployment}
    \begin{itemize}
        \item Build production-ready web application
        \item Next.js, REST API, live predictions
    \end{itemize}
    
    \item \textbf{Research Readiness}
    \begin{itemize}
        \item Reproducible experiments, version control, documentation
        \item Prepare for PhD-level research methodology
    \end{itemize}
\end{enumerate}
\end{frame}
